...
def random_rotation(x1, x2, x3, y, rg, row_axis=1, col_axis=2, channel_axis=0,
                    fill_mode='nearest', cval=0.):
    """Performs a random rotation of a Numpy image tensor.
    # Arguments
        xi: Input tensor. Must be 3D.
        y: Input tensor. Must be 3D.
    ...
    # Returns
        Rotated Numpy image and mask tensors.
    """
    ...
    x1 = apply_transform(x1, transform_matrix, channel_axis, fill_mode, cval)
    x2 = apply_transform(x2, transform_matrix, channel_axis, fill_mode, cval)
    x3 = apply_transform(x3, transform_matrix, channel_axis, fill_mode, cval)
...
def random_shift(x1, x2, x3, y, wrg, hrg, row_axis=1, col_axis=2, channel_axis=0,
                 fill_mode='nearest', cval=0.):
    ...
    # Arguments
        ...
        cval: Value used for points outside the boundaries
            of the input if `mode='constant'`.
    # Returns
        Shifted Numpy image and mask tensors.
    """
    h, w = x1.shape[row_axis], x1.shape[col_axis]
...
def random_shear(x1, x2, x3, y, intensity, row_axis=1, col_axis=2, channel_axis=0,
                 fill_mode='nearest', cval=0.):
    ...
    # Arguments
        ...
        cval: Value used for points outside the boundaries
            of the input if `mode='constant'`.
    # Returns
        Sheared Numpy image and mask tensors
    """
...
def random_zoom(x1, x2, x3, y, zoom_range, row_axis=1, col_axis=2, channel_axis=0,
                fill_mode='nearest', cval=0.):
    ...
    # Arguments
        ...
        cval: Value used for points outside the boundaries
            of the input if `mode='constant'`.
    # Returns
        Zoomed Numpy image and mask tensors
    # Raises
...
def elastic_deform(x1, x2, x3, mask, alpha, sigma, random_state=None):
    """
    ...
    distored_x1 = map_coordinates(x1, indices, order=1, mode='reflect')
    distored_x2 = map_coordinates(x2, indices, order=1, mode='reflect')
    distored_x3 = map_coordinates(x3, indices, order=1, mode='reflect')
    distored_mask = map_coordinates(mask, indices, order=1, mode='reflect')
    return distored_x1.reshape(x1.shape), distored_x2.reshape(x1.shape), distored_x3.reshape(x1.shape), distored_mask.reshape(x1.shape)
...
class customImageDataGenerator(object):
    ...
    def random_transform(self, x, y, seed=None):
        ...
        if self.elastic_deform:
            x = elastic_deform(
                x, alpha=self.elastic_deform[0], sigma=self.elastic_deform[1], random_state=seed)
            y = elastic_deform(
                y, alpha=self.elastic_deform[0], sigma=self.elastic_deform[1], random_state=seed)
        ...
        if self.horizontal_flip:
            if np.random.random() < 0.5:
                x, y = flip_axis(x, y, img_col_axis)

        if self.vertical_flip:
            if np.random.random() < 0.5:
                x, y = flip_axis(x, y, img_row_axis)

        if self.random_mult_range != 0:
            if np.random.random() < 0.5:
                x = random_mutiplication(x, self.random_mult_range)
                y = random_mutiplication(y, self.random_mult_range)
        ...
        if self.brightness_range is not None:
            if np.random.random() < 0.5:
                ...
                y = apply_brightness_shift(y, brightness)
...
#                 print("self.x[j,:,:,s,:]:, ",x.shape)
                x, y = self.image_data_generator.random_transform(
                    x.astype(K.floatx()), y.astype(K.floatx()), seed=(j+1)*seed_random)
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                mask = array_to_img(batch_y[i], self.data_format, scale=True)
                fname = '{prefix}_{index}_{hash}.{format}'.format(prefix=self.save_prefix,
                                                                  index=current_index + i,
                                                                  hash=np.random.randint(
                                                                      1e4),
                                                                  format=self.save_format)
                fname_mask = '{prefix}_{index}_{hash}.{format}'.format(prefix=self.save_prefix + str('mask'),
                                                                  index=current_index + i,
                                                                  hash=np.random.randint(
                                                                      1e4),
                                                                  format=self.save_format)
